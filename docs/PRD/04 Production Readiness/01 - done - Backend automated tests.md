...
Gated in CI — see [10 - done - CI pipeline and deploy](./10%20-%20done%20-%20CI%20pipeline%20and%20deploy.md): `.github/workflows/ci.yaml`'s `test` job runs this suite on every PR and on push to `main`, and `deploy` only runs if `test` passes.
