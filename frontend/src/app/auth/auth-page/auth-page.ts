...
export class AuthPage implements OnInit {
    ...
    async ngOnInit() {
        ...
        if (this.mode === 'callback') {
            if (await this.auth.refresh()) await this.router.navigateByUrl('/'); else this.error = 'Sign-in failed. Please return to sign in.';
        }
        if (this.mode === 'login') {
            try {
                this.providers = await firstValueFrom(this.http.get<Record<string, boolean>>('/api/auth/providers'));
            } catch {
    ...
    async submit() {
        ...
        try {
            if (this.mode === 'register') {
                ...
                await this.auth.action('register', {
                    email: this.email,
                ...
                if (!this.validPassword || this.password !== this.confirmation) throw new Error('Use a password meeting the policy and matching confirmation.');
                this.message = (await this.auth.action('reset-password', {
                    token: this.token,
                    password: this.password,
                    password_confirmation: this.confirmation
                })).message;
                ...
                await this.auth.action('verify-email', {token: this.token});
                this.done = true;
                this.message = 'Your email is verified. You can now create meeting notes.';
            }
            this.password = '';
            this.confirmation = '';
        } catch (e) {
